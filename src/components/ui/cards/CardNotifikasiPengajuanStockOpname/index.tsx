...
import type { ResponseRiwayatStockOpnameForNotifikasiType } from "../../../../models/riwayatStockOpname.model";
import { stockOpnameNotification } from "../../../../types/constant.type";
...
const CardNotifikasiStockOpname: FC<Props> = ({
  data: { author, updatedAt, keterangan, status, stockOpnameId },
  large,
  windowSize,
  handleRedirectDetail,
}) => {
  const Icon = stockOpnameNotification[status].icon;
      ...
      {large && windowSize !== "sm" ? (
        <div className="w-full h-full p-2.5 flex flex-row justify-between items-center gap-2.5">
          <div className="flex-3 lg:flex-6 w-full flex flex-row justify-start items-center gap-4">
            ...
            <div
              className={cn(
                ...
                stockOpnameNotification[status].bg,
              )}
            >
              <Icon
                className={cn("size-4", stockOpnameNotification[status].color)}
              />
            </div>
            ...
            <div className="flex flex-col justify-start items-start">
              <div className="flex flex-col justify-start items-start gap-0.5">
                <div className="flex gap-2.5 flex-row justify-start items-center">
                  <span
                    className={cn(
                      "status",
                      stockOpnameNotification[status].bullet,
                    )}
                  />

                  <span className="text-[0.7rem] font-medium text-base-content">
                    {stockOpnameNotification[status].title}
                  </span>
                </div>
              ...
              <div className="w-full flex flex-row justify-start items-center gap-2 mt-2">
                ...
                <span
                  className={cn(
                    "text-[0.625rem] text-base-content font-medium",
                  )}
                >
                  Stock Opname
                </span>
          ...
          <div className="flex-1 flex flex-row justify-end items-center">
            <ButtonWithIcon
              icon={Eye}
              label="Lihat Detail"
              handleBtn={() => handleRedirectDetail(stockOpnameId)}
            />
          </div>
        </div>
      ) : (
        ((large && windowSize === "sm") || !large) && (
          <button
            ...
            onClick={() => handleRedirectDetail(stockOpnameId)}
          >
            <div className="w-full flex flex-row justify-start items-center gap-4">
              ...
              >
                <Icon
                  className={cn(
                    "size-4",
                    stockOpnameNotification[status].color,
                  )}
                />
              </div>
...
export default CardNotifikasiStockOpname;
