...
import { statusStyleStockOpname } from "../../types/statusStyle";
import PaginationAndLimit from "../filters/PaginationAndLimit";
import useContentSideBar from "../../hooks/useContentSideBar";
import useSideBarRiwayatPengajuanStockOpname from "./useSideBarRiwayatPengajuanStockOpname";

const SideBarRiwayatPengajuanStockOpname = () => {
  // call use
  const { dataRiwayat, isExistDataRiwayat, isLoadingRiwayat, setPage } =
    useSideBarRiwayatPengajuanStockOpname();
                ...
                <div className="w-full flex-12 scrollbar-thumb-custom-secondary overflow-y-auto flex flex-col justify-start items-start">
                  ...
                  {isLoadingRiwayat ? (
                    ...
                    dataRiwayat?.data?.data.map((item, index) => {
                      const style = statusStyleStockOpname[item.status];
                      const isLast =
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative flex justify-center">
                            {!isLast && (
                              <div className="absolute top-6.5 h-[88%] -bottom-6 left-1/2 w-px -translate-x-1/2 bg-base-content/20" />
                            )}
                            ...
                            >
                              <div
                                className={cn(
                                  "w-2.5 h-2.5 md:w-2 md:h-2 rounded-full bg-amber-600",
                                  style.dot,
                                )}
                              />
                            </div>
...
export default SideBarRiwayatPengajuanStockOpname;
